...
export const config: Options.Testrunner = {
    ...
    //     browser.options["enviroment"] = config.enviroment
    //     browser.options["amazonPage"] = config.amazonPage
     ...
     */
     beforeScenario: function (world) {
        ...
        //regular expression , array of strings , test id & scenario :) --> 
        let arr = world.pickle.name.split(/:/)
        // @ts-ignore
        if (arr.length > 0) browser.options.testid = arr[0]
         // @ts-ignore
        if (!browser.options.testid) throw Error (`Error getting for current scenario: ${world.pickle.name}`)
    ...
    afterStep: async function (step, scenario, result, context) {
        ...
        if(!result.passed){
            ...
            console.log(`>> step: ${JSON.stringify(scenario)}`);
